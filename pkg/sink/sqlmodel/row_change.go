...
type RowChange struct {
	sourceTable *common.TableName
	targetTable *common.TableName

	preValues  []interface{}
	postValues []interface{}

	sourceTableInfo *common.TableInfo
	targetTableInfo *common.TableInfo
...
func NewRowChange(
	...
	postValues []interface{},
	sourceTableInfo *common.TableInfo,
	downstreamTableInfo *common.TableInfo,
...
func (r *RowChange) whereColumnsAndValues() ([]string, []interface{}) {
	...
	columns, values := r.sourceTableInfo.GetColumns(), r.preValues

	uniqueIndex := r.whereHandle.getWhereIdxByData(r.preValues)
	if uniqueIndex != nil {
		columns, values = getColsAndValuesOfIdx(r.sourceTableInfo.GetColumns(), uniqueIndex, values)
	}
...
func (r *RowChange) genUpdateSQL() (string, []interface{}) {
	...
	generatedColumns := generatedColumnsNameSet(r.targetTableInfo.GetColumns())
	args := make([]interface{}, 0, len(r.preValues)+len(r.postValues))
	writtenFirstCol := false
	for i, col := range r.sourceTableInfo.GetColumns() {
...
func (r *RowChange) GetSourceTable() *common.TableName {
	return r.sourceTable
}

// GetTargetTable returns TableName of the target table.
func (r *RowChange) GetTargetTable() *common.TableName {
