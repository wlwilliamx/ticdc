...
func GetWhereHandle(source, target *common.TableInfo) *WhereHandle {
	ret := WhereHandle{}
	indices := make([]*model.IndexInfo, 0, len(target.GetIndices())+1)
	indices = append(indices, target.GetIndices()...) // Updated to use GetIndices()
	...
	for _, idx := range indices {
		...
		if ret.UniqueNotNullIdx == nil && allColsNotNull(idx, target.GetColumns()) {
			ret.UniqueNotNullIdx = rewritten
			continue
		}
	}
...
func rewriteColsOffset(index *model.IndexInfo, source *common.TableInfo) *model.IndexInfo {
	...
	for _, key := range index.Columns {
		sourceColumn := model.FindColumnInfo(source.GetColumns(), key.Name.L)
		if sourceColumn == nil {
			return nil
		}
...
func getPKIsHandleIdx(ti *common.TableInfo) *model.IndexInfo {
	if pk := ti.GetPkColInfo(); pk != nil {
		return &model.IndexInfo{
			Table:   pmodel.CIStr{O: ti.TableName.Table, L: ti.TableName.Table},
			Unique:  true,
			Primary: true,
