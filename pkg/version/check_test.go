...
func TestCompareVersion(t *testing.T) {
	...
	masterVersion := semver.New(SanitizeVersion("v7.5.0-master"))
	require.Equal(t, 1, masterVersion.Compare(*MinTiCDCVersion))

	// pre-release version, `vx.y.z-alpha-nightly-yyyymmdd`
	alphaVersion := semver.New(SanitizeVersion("v7.5.0-alpha-nightly-20220202"))
	require.Equal(t, 1, alphaVersion.Compare(*MinTiCDCVersion))

	// release version, `vx.y.z.`
	releaseVersion := semver.New(SanitizeVersion("v7.5.0"))
	require.Equal(t, 1, releaseVersion.Compare(*MinTiCDCVersion))

	// build with uncommitted changes, `vx.y.z-dirty`
	dirtyVersion := semver.New(SanitizeVersion("v7.5.0-dirty"))
	require.Equal(t, 1, dirtyVersion.Compare(*MinTiCDCVersion))
	require.Equal(t, 0, dirtyVersion.Compare(*semver.New("7.5.0")))

	dirtyVersionWithFIPS := semver.New(SanitizeVersion("v7.5.0-dirty-fips"))
	require.Equal(t, 1, dirtyVersionWithFIPS.Compare(*MinTiCDCVersion))
	require.Equal(t, 0, dirtyVersionWithFIPS.Compare(*semver.New("7.5.0")))
}
